<template>
  <div>
    <p class="text-xs font-semibold uppercase text-teal-700" style="letter-spacing: 0.24em;">相場マップ</p>
    <h1 class="mt-3 text-2xl font-semibold text-slate-900">マレーシアのコンド相場</h1>
    <p class="mt-3 text-sm leading-6 text-slate-600">
      州と郵便番号エリアを切り替えながら、相場帯と取引状況を直感的に確認します。
    </p>
      ...
      <div>
        ...
        <div class="mt-2 grid grid-cols-2 gap-2">
          <button
            ...
            :class="areaLevel === 'state' ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white text-slate-700'"
            type="button"
          ...
          <button
            class="rounded-full px-4 py-2 text-sm font-medium"
            :class="areaLevel === 'postal_code_area' ? 'bg-slate-900 text-white' : 'border border-slate-300 bg-white text-slate-700'"
            type="button"
            @click="$emit('update:areaLevel', 'postal_code_area')"
          >
            郵便番号エリア
          </button>
        </div>
      ...
      <div>
        ...
        <div class="mt-2 flex flex-wrap gap-2">
          <button
            ...
            :class="priceBand === option.value ? 'bg-teal-700 text-white' : 'border border-slate-300 bg-white text-slate-700'"
            type="button"
